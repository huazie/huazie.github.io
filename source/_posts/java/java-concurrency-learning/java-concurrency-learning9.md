...
我们首先能想到的就是，通过 `HashMap` 和**并发机制**来构建缓存，代码示例如下：
...
本篇所有示例代码地址 [请点击这里](https://github.com/Huazie/FleaJavaConcurrency/tree/main/basic-building-module-demo)，其中的 `Servlet` 可以通过 `JettyStarter` 启动服务端，然后浏览器访问 <http://localhost:8080/memoizer?factor=1231231234> 或者 使用 **JMeter** 模拟多用户高并发请求。
