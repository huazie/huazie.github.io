...
-  在 `META-INF/spring.factories` 中添加 `org.springframework.context.ApplicationContextInitializer` 的配置。这种方式，我们从 [《ApplicationContextInitializer 详解》](../../../../../2023/12/03/spring-boot/spring-boot-sourcecode-applicationcontextinitializer/) 的 **2.2** 小节可见一斑。
  ```bash
...
- 通过 `SpringApplication` 中的 `addInitializers` 方法添加。其实这里在笔者的[《SpringApplication 的定制化介绍》](../../../../../2024/01/07/spring-boot/spring-boot-sourcecode-springapplication-customization/)中的 **1.6** 小节也提及过。
  ...
  // 其他省略。。。
  ```
- 在 **application.properties** 中添加 `context.initializer.classes` 的属性配置。这里实际上来源于 **2.1.3** 小节的 **DelegatingApplicationContextInitializer**。
  ```bash            
  # 逗号分隔的类名列表
