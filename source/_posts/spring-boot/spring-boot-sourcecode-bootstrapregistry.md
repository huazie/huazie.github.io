...
在 [《BootstrapRegistryInitializer 详解》](../../../../../2023/11/30/spring-boot/spring-boot-sourcecode-bootstrapregistryinitializer/) 的 2.1 小节，我们对 `BootstrapRegistry` 进行了初步的介绍：它是一个用于存储和共享对象的注册表，这些对象在 `ApplicationContext` 准备好之前就可能已经被创建并需要被共享。
## 2.1 源码初识
